    ...
    return (
        <section className='login information'>
            ...
            <form className='information__form' onSubmit={handleSubmit(submit)}>
                ...
                <input className='information__form-input' type='email' required name='email'
                       id='email' {...register('email', {
                    required: true,
                    pattern: /([A-z0-9_.-]{1,})@([A-z0-9_.-]{1,}).([A-z]{2,8})/
                })}/>
                <span
                    className='information__form-input-error'>{errors.email?.type === "required" && "Пожалуйста, заполните поле"}
                ...
                <input className='information__form-input' type='password' required name='password'
                       id='password'  {...register('password', {required: true})}/>
                <span
                    className='information__form-input-error'>{errors.password?.type === "required" && "Пожалуйста, заполните поле"}</span>
                <button className={'information__button login__button' + (!isValid ? ' form__button_disabled' : '')}
